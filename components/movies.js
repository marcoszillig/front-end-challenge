...
const MovieLink = () => {
  ...
  const { movies, genreID, total_pages } = moviesState;

  const moviesFiltered = movies.filter((movie) => {
    return genreID ? movie.genre_ids.includes(genreID) : movie;
  });
  
  return moviesFiltered.length > 0 ? (
    ...
    <h3 className="movie-list-msg">Nenhum filme foi encontrado
      <style jsx>{`
        .movie-list-msg {
          font-family: "Poppins", sans-serif;
          font-size: 2rem;
